...
detecterCreneauDisponibleV1();

function detecterCreneauDisponibleV1() {
  const debutJournee = "07:59";
  const finJournee = "18:00";
  const horaireDebut1 = lireHoraireDebutDuPremierHoraire();
  const horaireFin1 = lireHoraireFinDuPremierHoraire();

  // Est-ce qu'il y a un trou d'au moins 60 minutes entre 
  // le début de journée et le début du premier horaire ?
  if (dureeTrou(debutJournee, horaireDebut1) >= 60) {
    afficherReponse(debutJournee);
    return;
  }

  const horaireDebut2 = lireHoraireDebutDuDeuxiemeHoraire();
  const horaireFin2 = lireHoraireFinDuDeuxiemeHoraire();

  // Est-ce qu'il y a un trou d'au moins 60 minutes entre 
  // la fin du premier horaire et le début du 2ème horaire ?
  if (dureeTrou(horaireFin1, horaireDebut2) >= 60) {
  ...
  if (dureeTrou(horaireFin2, finJournee) >= 60) {
...
function lireHoraireDebutDuDeuxiemeHoraire() {
  return "10:45";
}

function lireHoraireFinDuDeuxiemeHoraire() {
  return "14:47";
}
